...
function _make_source(g::StaticInjection, r::Float64, x::Float64, sys_base::Float64)
    ...
    machine_base = get_base_power(g, u"NU")
    ...
    return Source(;
        ...
        available = true,
        bus = get_bus(g),
        active_power = get_active_power(g, u"SU"),
        reactive_power = get_reactive_power(g, u"SU"),
        R_th = r_sysbase,
        X_th = x_sysbase,
        input_basis = u"CU",
    )
...
function _add_dyn_injectors!(
    ...
    for g in get_components(InjectorType, sys)
        ...
        if temp_dict === nothing
            ...
            s = _make_source(g, r, x, get_base_power(sys, u"NU"))
