...
@testset "PSSE v30 unparsed sections warn and are skipped" begin
    # synthetic_data_v30.raw carries 3W-transformer, switched-shunt, and DC-line
    # records that the PowerFlowData-based v30 path does not parse; the build must
    # surface each documented data drop as a warning and still return a System.
    sys = @test_logs (
        :warn,
        r"Three-winding transformer from PowerFlowData inputs not implemented",
    ) (:warn, r"TwoTerminalDCLines parsing from PSS/e v30 files not implemented") match_mode =
        :any build_system(PSSEParsingTestSystems, "PSSE 30 Test System"; force_build = true)
